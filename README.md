...
HTML と CSS を中心とした静的サイトです。フレームワークやビルドは不要で、JavaScript はテーマ切り替えとフッターの年表示の更新に使用しています。

| ファイル | 内容 |
| --- | --- |
| `index.html` | プロフィール、研究内容、発表実績、開発・技術経験、外部リンク |
| `theme.js` | ライト／ダーク切り替え、設定の保存・復元 |
...
ヘッダーのボタンでライト／ダークを切り替えられます。初回は端末の配色設定に合わせ、選択後はブラウザに保存した設定を優先します。保存が制限されている場合も、そのページ内での切り替えは利用できます。
